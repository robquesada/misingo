...
describe FlyersController do
  ...
  describe "GET show" do
    ...
    let(:flyers_generator) { double("PetProject::Flyers::Generator", generate_flyer: flyer) }

    before do
      allow(PetProject::Flyers::Generator).to receive(:new) { flyers_generator }
    end
    ...
    describe "generating the flyer" do
      it "creates a generator" do
        expect(PetProject::Flyers::Generator).to receive(:new).with(pet, lost_report)
        get :show, pet_id: pet.id
      end

      it "generates the flyer" do
        expect(flyers_generator).to receive(:generate_flyer)
        get :show, pet_id: pet.id
      end
    end
